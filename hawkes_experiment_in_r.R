...
experiment_truncate_time <- function(test_number, time_interval, number_of_simulations, baseline, alpha, beta, number_of_deltas, min_delta, max_delta, duplicates, opt_method, init_estimation){
  ...
  for(i in 1:number_of_simulations){
    ...
    original_process = hawkes(end = time_interval, fun = baseline, repr = alpha/beta, family = "exp", rate = beta)
    ...
    for(truncate_delta_experiment in 1:number_of_deltas){
      ...
      if(opt_method == "mle" || opt_method == "both"){
        ...
        if(init_estimation == "original_parameters"){
          parameters_estimation_mle = try(mle(truncated_timestamps , "Exponential", original_process$end, opts = list("algorithm"="NLOPT_LD_LBFGS", "xtol_rel" = 1.0e-8), init = c(baseline, alpha/beta, beta)))
        }
        else
      ...
      if(opt_method == "whittle" || opt_method == "both"){
        if(truncate_delta_experiment > 0){
          ...
          if(init_estimation == "original_parameters")
            parameters_estimation_whittle  = try(whittle(y, "Exponential", init = c(baseline, alpha/beta, beta)))
          else
      ...
      if(opt_method == "both"){
        ...
        list_to_csv_mle <- list(test_number, i, alpha, beta, baseline, alpha/beta, delta_vector[truncate_delta_experiment], estimated_parameters_mle[2], estimated_parameters_mle[3], estimated_parameters_mle[1], duplicates, "mle", init_estimation)
        
        ## Writing Whittle data to the csv file
        list_to_csv_whittle <- list(test_number, i, alpha, beta, baseline, alpha/beta, delta_vector[truncate_delta_experiment], estimated_parameters_whittle[2], estimated_parameters_whittle[3], estimated_parameters_whittle[1], duplicates, "whittle", init_estimation)
...
list_column_names_to_csv_parameters <- list("Test Index", "Simulation Index", "Real Alpha", "Real Beta", "Real Baseline", "Real Endogeneity", "Delta", "Estimated Endogeneity", "Estimated Beta", "Estimated Baseline", "Duplicates", "Estimation Method", "Initial Estimation Point")
